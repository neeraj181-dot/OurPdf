...
import { demoLogin, demoSignup, demoGoogleSignIn, DEMO_USERS } from "../lib/auth";
...
/**
 * Standalone full-page login UI with email/password, demo shortcuts,
 * and a simulated Google account picker for development.
 *
 * TODO: PRODUCTION — Replace demo auth calls with real apiLogin/apiRegister
 * from ../lib/api.ts, and replace the Google picker with real Google Identity Services.
 */
export const LoginPage: React.FC<LoginPageProps> = ({
  ...
  const handleGoogleSignIn = (selectedEmail?: string, selectedName?: string) => {
    ...
    setTimeout(() => {
      const res = demoGoogleSignIn(selectedEmail, selectedName);
      if (res.user) {
        soundEffects.playSuccess();
        setSuccessMsg(`Signed in with Google as ${res.user.email}`);
        setTimeout(() => onAuthSuccess(res.user!), 500);
      }
      setIsLoading(false);
    }, 400);
  };
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    setTimeout(() => {
      if (isSignUp) {
        const res = demoSignup(name, email, password);
  ...
  return (
    <div className="min-h-full w-full bg-[#121215] flex flex-col items-center justify-center p-4 sm:p-8 font-sans text-zinc-100 relative">
        ...
        <div className="p-6 sm:p-8 flex flex-col justify-between">
          <div>
            ...
            <div className="flex items-center gap-3 mb-6">
              ...
              <div>
                <h1 className="text-lg font-extrabold tracking-tight text-zinc-100">PDFora Studio</h1>
                <p className="text-xs text-zinc-400">Document Intelligence Platform</p>
              </div>
            </div>
            ...
            <p className="text-xs text-zinc-400 mb-6">
              {isSignUp
                ? "Sign up to unlock all PDF tools & cloud storage"
                : "Sign in to manage your documents & cloud workspace"}
            </p>
            ...
            <div className="space-y-3 mb-5">
              <button
                ...
                disabled={isLoading}
                className="w-full flex items-center justify-center gap-3 bg-white hover:bg-zinc-100 text-zinc-800 font-bold text-xs py-3 px-4 rounded-xl border border-zinc-300 transition-all cursor-pointer shadow-md active:scale-[0.99]"
              >
                ...
                <span className="text-[9px] text-zinc-400 font-normal ml-1">(Demo)</span>
              </button>
        ...
        <div className="bg-gradient-to-br from-[#1DB954]/20 via-zinc-900 to-black p-8 flex flex-col justify-between border-t md:border-t-0 md:border-l border-zinc-800 relative overflow-hidden">
          <div className="relative z-10">
            <div className="inline-flex items-center gap-1.5 px-3 py-1 rounded-full bg-[#1DB954]/20 border border-[#1DB954]/40 text-[#1DB954] text-xs font-bold mb-6">
              <Sparkles className="w-3.5 h-3.5" /> DOCUMENT INTELLIGENCE
            </div>
            ...
            <div className="space-y-3.5">
              <div className="flex items-start gap-3 text-xs text-zinc-300">
                ...
                <div>
                  <span className="font-semibold text-zinc-100">Cloud Document Storage</span>
                  <p className="text-zinc-400 text-[11px]">Save & access your documents from any device</p>
                </div>
              </div>
              ...
              <div className="flex items-start gap-3 text-xs text-zinc-300">
                ...
                <div>
                  ...
                  <p className="text-zinc-400 text-[11px]">Create, edit, merge, organize, watermark, annotate & convert</p>
                </div>
              </div>
            </div>
          </div>
      ...
      {/* Google Account Picker Modal (Demo Simulator) */}
      <AnimatePresence>
            ...
            >
              <div className="flex items-center justify-between mb-4">
                <div className="flex items-center gap-2">
                  ...
                  <span className="font-bold text-sm">Sign in with Google <span className="text-[10px] text-zinc-500 font-normal">(Demo)</span></span>
                </div>
                <button
                  onClick={() => {
                    setShowGoogleAccountPicker(false);
              ...
              {!isCustomGoogleAccount ? (
                <>
                  <p className="text-xs text-zinc-400 mb-4">Choose an account to continue to PDFora:</p>
                  <div className="space-y-2">
                    <button
                      onClick={() => {
                        setShowGoogleAccountPicker(false);
                        handleGoogleSignIn("alex.demo@gmail.com", "Alex Rivera");
                      }}
                    ...
                    >
                      <div className="flex items-center gap-3">
                        <div className="w-8 h-8 rounded-full bg-[#4285F4] text-white font-bold flex items-center justify-center text-xs">A</div>
                        <div>
                          <div className="text-xs font-semibold text-zinc-100">Alex Rivera</div>
                          <div className="text-[11px] text-zinc-400">alex.demo@gmail.com</div>
                        </div>
                      </div>
                    ...
                    <button
                      onClick={() => {
                        setShowGoogleAccountPicker(false);
                        handleGoogleSignIn("sarah.demo@gmail.com", "Sarah Chen");
                      }}
                      className="w-full flex items-center justify-between p-3 rounded-xl bg-zinc-800/80 hover:bg-zinc-700 text-left transition-colors cursor-pointer border border-zinc-700/60"
                    >
                      <div className="flex items-center gap-3">
                        <div className="w-8 h-8 rounded-full bg-[#EA4335] text-white font-bold flex items-center justify-center text-xs">S</div>
                        <div>
                          <div className="text-xs font-semibold text-zinc-100">Sarah Chen</div>
                          <div className="text-[11px] text-zinc-400">sarah.demo@gmail.com</div>
                        </div>
                      </div>
                      <ChevronRight className="w-4 h-4 text-zinc-400" />
                    </button>
                ...
                <form onSubmit={handleCustomGoogleSubmit} className="space-y-3">
                  <p className="text-xs text-zinc-400 mb-2">Enter your account details (Demo):</p>
                  <div>
